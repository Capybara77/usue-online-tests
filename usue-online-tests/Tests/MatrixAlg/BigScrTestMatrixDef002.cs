...
namespace UserTest
{
    public class BigScrTestMatrixDef002 : ITestCreator, ITest, ITimeLimit, IHidden, ITestGroup
    {
        public int TestID { get; set; }
        public string GroupName { get; set; }= "Matrix Algebra";
        public string Name { get; } = "Определение матрицы 002";
        ...
        public class Data
        {
            ...
            public void CreateMatrixes(Random random)
            {
                ...
                //    answA[i] = MatrA[i];
                //}
                answA[0] = 3*MatrB[0]+MatrC[0];
                answA[1] = 3*MatrB[0]+MatrC[1];
                answA[2] = 3*MatrB[0]+MatrC[2];
                answA[3] = 3*MatrB[1]+MatrD[0];
                answA[4] = 3*MatrB[1]+MatrD[1];
                answA[5] = 3*MatrB[1]+MatrD[2];
            }
        }
        public ITest CreateTest(int randomSeed)
        {
            ITest result = new BigScrTestMatrixDef002();
            ...
            result.Text =
                $" Пусть \\(\\left(\\begin{{array}}{{ccc}} {data.MatrA[0]} & {data.MatrA[1]} & {data.MatrA[2]}\\\\ " +
                $" {data.MatrA[3]} & {data.MatrA[4]} & {data.MatrA[5]}\\\\ \\end{{array}}\\right)\\). Тогда\r\n" +
                $"\\({data.MCb}_{{{data.MatrB[0]+1},{data.MatrC[0]+1}}}= <ans[{data.answA[0]}]:5>,\\quad\\) " +
                $"\\({data.MCb}_{{{data.MatrB[0]+1},{data.MatrC[1]+1}}}= <ans[{data.answA[1]}]:5>,\\quad\\) " +
                $"\\({data.MCb}_{{{data.MatrB[0]+1},{data.MatrC[2]+1}}}= <ans[{data.answA[2]}]:5>,\\quad\\) " +
                $"\\({data.MCb}_{{{data.MatrB[1]+1},{data.MatrD[0]+1}}}= <ans[{data.answA[3]}]:5>,\\quad\\) " +
                $"\\({data.MCb}_{{{data.MatrB[1]+1},{data.MatrD[1]+1}}}= <ans[{data.answA[4]}]:5>,\\quad\\) " +
                $"\\({data.MCb}_{{{data.MatrB[1]+1},{data.MatrD[2]+1}}}= <ans[{data.answA[5]}]:5>,\\quad\\)";
        ...
        public int CheckAnswer(int randomSeed, Dictionary<string, string> answers)
        {
            ...
            try
            {
                for (int i = 0; i < 6; i++)
                {
                    ...
                    if (answers["ans["+i+"]"] == data.MatrA[i].ToString()) total++;
                }
            }
            catch
            {
