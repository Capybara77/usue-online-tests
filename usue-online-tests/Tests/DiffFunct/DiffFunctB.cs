...
{
    public class DiffFunctB001 : ITestCreator, ITest, ITimeLimit, IHidden, ITestGroup
    {
        public int TestID { get; set; }
        public string GroupName { get; set; } = "Diff of function";
        public string Name { get; } = "Производная функции от линейной функции 02";
        ...
        public class Data
        {
            ...
            public void CreateFunct(Random random)
            {
                ...
                //Vvv = 1;
                FunctTeX = string.Concat(BasicFunct[4 * Vvv],Convert.ToString(kA,10),"x+",Convert.ToString(kB,10),BasicFunct[4 * Vvv+1]);
                FunctCalc = string.Concat("der(",BasicFunct[4 * Vvv + 2],Convert.ToString(kA,10),"*x+",Convert.ToString(kB,10),BasicFunct[4 * Vvv+3],", x)");
            }
        }

        public ITest CreateTest(int randomSeed)
        {
            ITest result = new DiffFunctB001();
