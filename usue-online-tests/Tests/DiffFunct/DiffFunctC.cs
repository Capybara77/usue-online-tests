...
{
    public class DiffFunctC001 : ITestCreator, ITest, ITimeLimit, IHidden, ITestGroup
    {
        public int TestID { get; set; }
        public string GroupName { get; set; } = "Diff of function";
        public string Name { get; } = "Производная суперпозиции функций 03";
        ...
        public class Data
        {
            ...
            public void CreateFunct(Random random)
            {
                ...
                //Vvv = 1;
                FunctTeX = string.Concat(BasicFunct[4 * Vvv],BasicFunctB[4 * Vvw],"x",BasicFunctB[4 * Vvw+1],BasicFunct[4 * Vvv+1]);
                FunctCalc = string.Concat("der(",BasicFunct[4 * Vvv + 2],BasicFunctB[4 * Vvw + 2],"x",BasicFunctB[4 * Vvw+3],BasicFunct[4 * Vvv+3],", x)");
            }
        }

        public ITest CreateTest(int randomSeed)
        {
            ITest result = new DiffFunctC001();
