...
namespace UserTest
{
    ...
    /// </summary>
    public class VectorCrossProductTest : ITestCreator, ITest, ITimeLimit, IHidden, ITestGroup
    {
        // Свойства теста
        public int TestID { get; set; }
        public string GroupName { get; set; } = "Analytic Geometry and Vectors"; // Название группы тестов
        public string Name { get; } = "Геометрическое вычисление векторного произведения 001"; // Название теста
        ...
        public class Data
        {
            ...
            public List<int> NumberA = new List<int> { 1, 2, 4, 5 };
            
            // Названия вершин параллелепипеда
            public string PAa, PBa, PCa, PDa, PAb, PBb, PCb, PDb;
            // Длины рёбер
            ...
            public Data(Random random)
            {
                ...
                AaBaAaCaX = 0;
                AaBaAaCaY = 0;
                AaBaAaCaZ = LAaBa * LAaCa / LAaAb;

                AaBaAaAbX = 0;
                AaBaAaAbZ = 0;
                AaBaAaAbY = -LAaBa * LAaAb / LAaCa;

                AaCaAaAbY = 0;
                AaCaAaAbZ = 0;
                AaCaAaAbX = LAaCa * LAaAb / LAaBa;
            }
        }
        ...
        public ITest CreateTest(int randomSeed)
        {
            ...
            Bitmap img = new Bitmap(600, 500);
using (Graphics graphics = Graphics.FromImage(img))
{
    graphics.SmoothingMode = SmoothingMode.AntiAlias;
    graphics.Clear(Color.White);

    int width = data.LAaBa * 50;
    int height = data.LAaCa * 50;
    int depth = data.LAaAb * 50;
    int xOffset = 150;
    int yOffset = 150;
    ...
    Point vectorCEnd = new Point(vectorCStart.X + (vectorC[2] + depth / 2), vectorCStart.Y - (vectorC[2] + depth / 2));
    ...
    using (Brush labelBrush = new SolidBrush(Color.Black))
    {
        ...
        Point vectorAEnd = new Point(vectorAStart.X + vectorA[0] * 50, vectorAStart.Y + width/4 - vectorA[1] * 50);
        Point vectorBStart = frontFace[3];
        Point vectorBEnd = new Point(vectorBStart.X + vectorB[0] * 50, vectorBStart.Y - vectorB[1] * 50);
        ...
        using (Pen vectorCPen = new Pen(Color.DarkGreen, 3) { CustomEndCap = new AdjustableArrowCap(6, 6) })
        {
            ...
            graphics.DrawString("a", labelFont, labelBrush, vectorAEnd.X + 5, vectorAEnd.Y - 15);
            graphics.DrawString("b", labelFont, labelBrush, vectorBEnd.X + 5, vectorBEnd.Y - 15);
            graphics.DrawString("c", labelFont, labelBrush, vectorCEnd.X + 5, vectorCEnd.Y - 15);
        }
    }
}
            ...
            test.Text = $"В прямоугольном параллелепипеде {data.PAa}{data.PBa}{data.PCa}{data.PDa}{data.PAb}{data.PBb}{data.PCb}{data.PDb} " +
                        $"имеем $|a| = {data.LAaBa}$, $|c| =  {data.LAaAb}$, " +
                        $"$|b| ={data.LAaCa}$. Тогда:\n" +
                        $"1. Векторное произведение $[\\vec{{{data.PAa}{data.PBa}}} \\times \\vec{{{data.PAa}{data.PCa}}}] $ = " +
                        ...
                        $"2. Векторное произведение $[\\vec{{{data.PAa}{data.PBa}}} \\times \\vec{{{data.PAa}{data.PAb}}}]$ = " +
                        ...
                        $"3. Векторное произведение $[\\vec{{{data.PAa}{data.PCa}}} \\times \\vec{{{data.PAa}{data.PAb}}}]$ = " +
