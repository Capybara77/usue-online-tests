...
namespace UserTest
{
    public class AnalyticGeometryProjectionFromA : ITestCreator, ITest, ITimeLimit, IHidden, ITestGroup
    {
        int answerCx;
        int answerCy;
        ...
        public string Name { get; } = "Проекция точки на прямую через точку A, ортогональную вектору";
        public string Description { get; } = "Проекция точки B на прямую, проходящую через A и ортогональную вектору";
        ...
        public ITest CreateTest(int randomSeed)
        {
            ...
            ITest test = new AnalyticGeometryProjectionFromA();
            ...
            using (Graphics g = Graphics.FromImage(img))
            {
                ...
                // g.FillEllipse(Brushes.Black, centerX + data.Bx * scale - 4, centerY - data.By * scale - 4, 8, 8);
                // g.DrawString($"{data.Bname} ({data.Bx},{data.By})", font, Brushes.Black, 
                //             centerX + data.Bx * scale + 5, centerY - data.By * scale - 20);
            ...
            test.Text =
                $"Прямая, проходящая через точку {data.Aname} ортогонально изображённому вектору, " +
