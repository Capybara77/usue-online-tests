...
{
    public class ComplexPlaneMultTest : ITestCreator, ITest, ITimeLimit, IHidden, ITestGroup
    {
        int answer1, answer2, answer3, answer4, answer5, answer6;

        public int TestID { get; set; }
        public string GroupName { get; set; } = "Complex Numbers";
        public string Name { get; } = "Умножение комплексных чисел в комплексной плоскости";
        public string Description { get; } = "Умножение комплексных чисел в комплексной плоскости";
        ...
        public ITest CreateTest(int randomSeed)
        {
            ...
            ITest test = new ComplexPlaneMultTest();
            ...
            if (Ax == 1)
            {
                Bx = random.Next(2, 7); // {2,3,4,5,6}
            }
            else if (Ax == 2)
            {
                Bx = random.Next(2, 4); // {2,3}
            }
            else if (Ax == 3)
            {
                Bx = random.Next(1, 3); // {1,2}
            }
            else if (Ax >= 4 && Ax <= 6)
            {
                Bx = 1; 
            ...
            answer1 = Ax * Bx;
            answer2 = Ay + By < 24 ? Ay + By : Ay + By - 24;
    ...
    using (Graphics graphics = Graphics.FromImage(img))
    {
               ...
               })
        {
            ...
            Point endB = new Point(
                centerX + (int)(Bx*50 * Math.Cos((15.0*By)* Math.PI / 180.0)),
                centerY - (int)(Bx*50 * Math.Sin((15.0*By)* Math.PI / 180.0))
            );
    ...
    string questionText = 
        $"На комплексной плоскости с изображённой полярной осью произведение чисел " +
        $"(векторов комплексной плоскости) \\(\\overrightarrow{{ {data.VectA} }} \\cdot \\overrightarrow{{ {data.VectB} }}\\) " +
