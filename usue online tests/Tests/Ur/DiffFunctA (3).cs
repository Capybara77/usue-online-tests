...
{
    public class DiffFunct002 : ITestCreator, ITest, ITimeLimit, IHidden, ITestGroup
    {
        public int TestID { get; set; }
        public string GroupName { get; set; } = "Diff of function";
        public string Name { get; } = "Таблица производных 02";
        public string Description { get; } = "Дифференцирование функций2";
        public class Data
        {
            //public string Wwx = "";
            //public string Wwy = "";
            ...
            string[] BasicFunct = new string[]
                {
                    ...
                    "\\sqrt{x}", "der(sqrt(x), x)", "\\arcsin{x}",  "der(asin(x), x)", "\\arccos{x}",  "der(acos(x))"
                };
        ...
        public ITest CreateTest(int randomSeed)
        {
            ...
            $" \\(\\textrm{{arctg}}\\,{{x}}\\) запишите как \\(\\texttt{{atg(x)}}.\\quad\\)\r\n ";
            //$"{data.Wwx}\r\n" +
            //$"{data.Wwy}\r\n";
            //$"{data.FunctCalc}";
            return result;
        ...
        public int CheckAnswer(int randomSeed, Dictionary<string, string> answers)
        {
            ...
            for (double i = -10; i < 10; i += 0.5)
            {
                ...
                try
                {
                    ...
                    //expression.addArguments(new Argument("x", i));
                    //userExpression.addArguments(new Argument("x", i/2));
                    //expression.addArguments(new Argument("x", i/2));

                    double expCorrect = expression.calculate();
                    double userAns = userExpression.calculate();
                    double results = expCorrect - userAns;

                    //if (double.IsNaN(expCorrect) == false) expCorrect=10^20;
                    //if (double.IsNaN(userAns) == false) userAns=10^20;

                    if ((double.IsNaN(expCorrect) != double.IsNaN(userAns)) ||
                        Math.Abs(results) > 0.0001)
                    {
                        return 0;
                    }

                    //if ( ((double.IsNaN(expCorrect) == false) && (double.IsNaN(userAns) == false) && (Math.Abs(results) > 0.0001)) || double.IsNaN(expCorrect) != double.IsNaN(userAns) )
                    //{
                    //    total = 0;
                    //    return 0;
                    //}
