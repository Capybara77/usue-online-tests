...
public class Profile : Controller
{
    ...
    public async Task<IActionResult> GetExams()
    {
        var user = await DataContext.Users
            .Include(user => user.UserExamResults)
            .FirstAsync(user => user.Id == GetUserByCookie.GetUser().Id);


        var res = DataContext.Users.Remove(user);
        await DataContext.SaveChangesAsync();

        return Json(user.UserExamResults.Select(result => new { result.Id, result.DateTimeStart }));
    }
